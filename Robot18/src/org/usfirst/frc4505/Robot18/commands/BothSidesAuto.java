...
public class BothSidesAuto extends Command {
    ...
    protected void initialize() {
    	...
    	tState = tStart;
    	correctColor = true;
    	RobotMap.rightSide = RobotMap.prefs.getBoolean("rightSide", RobotMap.rightSide);
    }
    ...
    protected void execute() {
    	while((Timer.getFPGATimestamp()-tStart) <= 15.0 && state != -1 && !Robot.oi.A.get()) {
    		if (state == 0) {//drive 80 inches
    			...
    			Robot.drivetrain.drive(0, (RobotMap.rightSide) ? -RobotMap.autoSpd : RobotMap.autoSpd);//check dir
    			if ((RobotMap.rightSide && (Robot.drivetrain.getAngle()-angle) <= -90.0) || (!RobotMap.rightSide && (Robot.drivetrain.getAngle()-angle) >= 90.0) || (Timer.getFPGATimestamp()-tState) > 3.0) {
    				Robot.drivetrain.stop();
    				tState = Timer.getFPGATimestamp();
    			...
    			Robot.drivetrain.drive(0,  (RobotMap.rightSide) ? RobotMap.autoSpd : -RobotMap.autoSpd);//check dir
    			if ((RobotMap.rightSide && (Robot.drivetrain.getAngle()-angle) >= 0.0) || (!RobotMap.rightSide && (Robot.drivetrain.getAngle()-angle) <= 0.0) || (Timer.getFPGATimestamp()-tState) > 3.0) {
    				Robot.drivetrain.stop();
    				tState = Timer.getFPGATimestamp();
    				Robot.drivetrain.resetAllTalons();
    				state++;
    			}
    		} else if (state == 5) {//drive 60 (CHECK) inches up to switch
    			Robot.drivetrain.driveStraight(-RobotMap.autoSpd, angle, Robot.drivetrain.getAngle(), 0.5);//check dir
